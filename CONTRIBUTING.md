...
| `app` | eslint (`pnpm lint`, zero problems); vitest; the three grading Lua scripts executed against a real Redis behind srh (`*.lua.upstash.test.ts`, required, not skippable, in CI); the production build; the `/`-never-prerendered assertion; the build-time config acceptance |
