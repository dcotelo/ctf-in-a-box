...
test('Challenge-42-Open-Redirect-Level-6', async () => {
  const res = await vaFetch('/Http3xxStatusCodeBasedInjection/LEVEL_6?returnTo=%2F%2Fevil.com', { redirect: 'manual' });
  // Anti-vacuous: prove the level actually evaluated the request before
  // trusting the absence assertion below (docs/scorer.md, #47).
  assertRedirected(res, 'challenge-42-open-redirect-level-6');
