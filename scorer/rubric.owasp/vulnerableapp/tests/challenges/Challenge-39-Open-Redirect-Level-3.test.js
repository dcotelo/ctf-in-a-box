...
test('Challenge-39-Open-Redirect-Level-3', async () => {
  const res = await vaFetch('/Http3xxStatusCodeBasedInjection/LEVEL_3?returnTo=/%09/evil.com', { redirect: 'manual' });
  // Anti-vacuous: prove the level actually evaluated the request before
  // trusting the absence assertion below (docs/scorer.md, #47).
  assertRedirected(res, 'challenge-39-open-redirect-level-3');
