...
// algorithm downgrade accepts forged HS256 token

test('Challenge-63-JWT-Level-16', async () => {
  const token = hs256({ alg: 'HS256', typ: 'JWT' }, PAYLOAD, HIGH_KEY);
  const res = await vaFetch('/JWTVulnerability/LEVEL_16', { headers: { Cookie: 'JWT=' + token } });
  // Anti-vacuous: prove the level actually evaluated the request before
  // trusting the absence assertion below (docs/scorer.md, #47).
  assertLevelResponded(res, 'challenge-63-jwt-level-16');
  assert.notEqual(res.json?.isValid, true, 'patched: algorithm-downgrade forged token must be rejected');
});
