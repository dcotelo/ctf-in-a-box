...
- Stripped Vercel-specific hosting bits (`@vercel/analytics`, `@vercel/oidc-aws-credentials-provider`, upstream's `.agents/`/`skills-lock.json` dev tooling) since the kit runs self-hosted via Docker, not Vercel; DynamoDB access now uses the plain AWS SDK default credential chain.
