...
// running the ai module right now. So this says that, instead of the root
// 404's "the link is just wrong or out of date" — which is true for a typo
// and false here. A contestant who had this page open a minute ago, or who
// followed a link an organizer posted this morning, has a correct link and a
// working browser; telling them otherwise sends them hunting for a better URL
// that does not exist.
//
// Enablement is a runtime setting (issue #175), so this is a state an event
// can enter and leave mid-flight. Nothing the contestant did caused it, and
// nothing they solved was lost by it.
//
// Inherits the `(site)` layout, so the nav and footer are already there — the
// body deliberately renders neither. Mirrors flags/not-found.tsx.

import NotFoundBody, { getNotFoundRoutes } from "@/components/not-found-body";
import { getAdminSettings } from "@/lib/admin-store";
import { moduleDefById } from "@/lib/modules";

export default async function AiNotFound() {
