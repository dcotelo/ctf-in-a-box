// 404 boundary for /quiz.
//
// The page calls `notFound()` for exactly ONE reason: this event is not
// running the quiz module right now. So this says that, instead of the
...
export default async function QuizNotFound() {
  const routes = await getNotFoundRoutes();
  const name = moduleDefById("quiz")?.displayName ?? "This module";
  return (
    <NotFoundBody
      routes={routes}
      eyebrow="Not running"
