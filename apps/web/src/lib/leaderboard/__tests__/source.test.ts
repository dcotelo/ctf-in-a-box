...
// board serves placeholder standings to real contestants. "dynamodb" for
// "dynamo" is the easy one to get wrong.

import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

vi.mock("server-only", () => ({}));
// The source modules reach for network and AWS clients at import time; none of
// that is exercised here, only which one gets picked.
...
vi.mock("@/lib/leaderboard/dynamo", () => ({ dynamoSource: { id: "dynamo" } }));
...
describe("getLeaderboardSourceMode", () => {
  it.each(["mock", "lambda", "dynamo", "upstash"])("passes %s through silently", async (value) => {
  ...
  it("warns and falls back to mock for the dynamodb/dynamo typo", async () => {
    const { getLeaderboardSourceMode } = await loadWith("dynamodb");
    expect(getLeaderboardSourceMode()).toBe("mock");
    expect(warn).toHaveBeenCalledTimes(1);

    const message = String(warn.mock.calls[0][0]);
    expect(message).toContain("dynamodb");
